...
import { ActionIcon, Card, Group, Image, Paper, Text } from '@mantine/core'
...
export const ItemModal = ({ item }: { item: ItemType | undefined }) => {
  return (
    <Card radius={12} padding={0}>
      <Image src={item?.image_url} mah={200} radius={12} />

      <Text fw={500} mt="xs" fz={20}>
        {item?.name}
      </Text>

      {item?.stop_list ? (
        <Paper bg="dark" c="white" p="xs">
          <Text fw={500}>Эта позиция в стоп листе</Text>
        </Paper>
      ) : (
        <>
